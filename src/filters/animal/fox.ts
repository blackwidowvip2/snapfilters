...
export function drawFox(d: DrawCtx) {
  ...
  [{ brow: lBrow, side: -1 }, { brow: rBrow, side: 1 }].forEach(({ brow, side }) => {
    ...
    ctx.rotate(angle + side * 0.14);

    const earGrad = ctx.createLinearGradient(0, -s * 0.38, 0, s * 0.08);
    earGrad.addColorStop(0, '#E55A00');
    earGrad.addColorStop(1, '#C04000');
    ctx.fillStyle = earGrad;
    ctx.beginPath();
    ctx.moveTo(0, -s * 0.38);
    ctx.lineTo(-side * s * 0.21, s * 0.08);
    ctx.lineTo(side * s * 0.21, s * 0.08);
    ctx.closePath();
    ctx.fill();
  ...
  [lCheek, rCheek].forEach((ch, ci) => {
    ...
    g.addColorStop(0, 'rgba(255,248,240,0.88)');
    g.addColorStop(0.5, 'rgba(255,240,220,0.55)');
    g.addColorStop(1, 'transparent');
    ctx.fillStyle = g;
    ctx.beginPath();
    ctx.ellipse(ch.x, ch.y, s * 0.22, s * 0.13, angle + (ci === 0 ? 0.15 : -0.15), 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  });
  ...
  ctx.rotate(angle);
  const noseGrad = ctx.createRadialGradient(-s * 0.018, -s * 0.018, 0, 0, 0, s * 0.075);
  noseGrad.addColorStop(0, '#1a0a00');
  noseGrad.addColorStop(1, '#0d0500');
  ctx.fillStyle = noseGrad;
  ctx.beginPath();
  ctx.ellipse(0, 0, s * 0.072, s * 0.052, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = 'rgba(255,255,255,0.22)';
  ctx.beginPath();
  ctx.ellipse(-s * 0.022, -s * 0.016, s * 0.02, s * 0.013, -0.4, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
  ...
  [{ side: -1, ox: -s * 0.04 }, { side: 1, ox: s * 0.04 }].forEach(({ side, ox }) => {
    for (let j = -1; j <= 1; j++) {
      ...
      ctx.beginPath();
      ctx.moveTo(0, 0);
      ctx.lineTo(side * s * 0.54, j * s * 0.05);
